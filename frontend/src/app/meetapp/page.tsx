...
export default function AppPage() {
  ...
  async function createMeeting() {
    ...
    setLoading(true); 
    setLoadingText("Creating meeting...");
    try {
      const res = await fetch(API_BASE + "/meetings", { 
        ...
        body: JSON.stringify({ 
          title: meetingTitle, 
          description: "Created via MeetTrack", 
  ...
  async function transcribeAudio() {
    ...
    setLoading(true); 
    setLoadingText("Transcribing audio with AI...");
    try {
  ...
  async function extractItems() {
    setLoading(true); 
    setLoadingText("AI extracting action items...");
    try {
  ...
  async function sendReminder(item: ActionItem) {
    try {
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        {loading && (
          <div className="text-center py-12">
            <p className="text-purple-300 text-lg">{loadingText}</p>
          </div>
        )}
        {!loading && step === 1 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <input 
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white mb-4" 
            />
            <button 
              onClick={createMeeting} 
              className="w-full bg-purple-600 hover:bg-purple-700 transition-colors text-white font-semibold py-3 rounded-xl"
            >
              Create Meeting
            </button>
          </div>
        ...
        {!loading && step === 2 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <div 
              className="border-2 border-dashed border-slate-700 rounded-xl p-10 text-center cursor-pointer mb-4 hover:border-purple-500/50 transition-colors" 
            ...
            <button 
              ...
              disabled={!audioFile} 
              className="w-full bg-purple-600 hover:bg-purple-700 transition-colors disabled:opacity-50 text-white font-semibold py-3 rounded-xl"
            >
              Transcribe with AI
            </button>
          </div>
        ...
        {!loading && step === 3 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <textarea 
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white h-48 resize-none mb-4" 
            />
            <button 
          ...
          <div>
                ...
                {actionItems.map((item, i) => (
                  <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex items-start justify-between gap-4">
                    ...
                    <button 
                      ...
                      className="bg-purple-600 hover:bg-purple-700 transition-colors text-white text-sm px-3 py-2 rounded-lg font-medium"
                    >
                      Remind
                    </button>
                  </div>
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 bg-slate-800 hover:bg-slate-700 transition-colors text-white font-semibold py-3 rounded-xl"
              >
                New Meeting
              </button>
              <a 
                href="/dashboard" 
                className="flex-1 bg-purple-600 hover:bg-purple-700 transition-colors text-white font-semibold py-3 rounded-xl text-center flex items-center justify-center"
              >
                Dashboard
              </a>
            </div>
